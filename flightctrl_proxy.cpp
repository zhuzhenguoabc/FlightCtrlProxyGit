...
int main(int argc, char* argv[])
{
	...
	*/
	...
	bool need_force_update_flightctrl_proxy = false;
	...
	{
		need_force_update_flightctrl_proxy = true;
	}
	*/
	...
	while (true)
	{
		...
		else if (need_force_update_flightctrl_proxy)
		{
			memset(result_to_client,0,MAX_BUFF_LEN);
		...
		*/
		...
		else
		{
			...
			*/
			...
			//strcat(drone_state_error, ":0");			//Normal state
			...
			if (bReceivedUdpMsg)
			{
				...
				else if ((gpsparams_udp.size() >= 1) && (gpsparams_udp[0].compare(SNAV_TASK_GET_SD_STATUS) == 0))
				{
					memset(result_to_client, 0, MAX_BUFF_LEN);
					...
					DEBUG("SNAV_TASK_GET_SD_STATUS_RETURN result_to_client=%s\n", result_to_client);

					length = sendto(server_udp_sockfd, result_to_client, strlen(result_to_client), 0,
										(struct sockaddr*)&remote_addr, sizeof(struct sockaddr));
					DEBUG("udp sendto SNAV_TASK_GET_SD_STATUS_RETURN length=%d\n", length);
				}
				...
				else if ((gpsparams_udp.size() >= 2) && (gpsparams_udp[0].compare("revise_height") == 0))
				{
					use_revise_height = atoi(gpsparams_udp[1].c_str());
			...
			distance_home_squared = (x_est_startup-x_est)*(x_est_startup-x_est)
			...
			else if(state == MissionState::TRAJECTORY_FOLLOW)
			{
				...
				if (return_mission)
				{
					if (fly_home)
					{
						...
						{
							// Go to home waypoint
							goto_waypoint({x_est, y_est, z_est, yaw_est}
		 									, {x_est_startup, y_est_startup, z_est, yaw_est}
					...
					else
					{
						...
						{
							goto_waypoint({x_est, y_est, z_est, yaw_est}
										, {x_est, y_est, z_est, yaw_target_home}
										, {gohome_x_vel_des, gohome_y_vel_des, gohome_z_vel_des, gohome_yaw_vel_des}
										, true, &output_vel, &wp_goal_ret);
						}
						else
			...
			if ((fabs(cmd0) < 1e-4 && fabs(cmd1) < 1e-4 ) || reverse_ctrl_count != 0)
			{
				if ((old_cmd0 != 0 || old_cmd1 != 0)  && reverse_ctrl_count < stop_control_num)
				{
					...
					if(reverse_ctrl_step == 2 && (fabs(old_cmd00) > 0.05 || fabs(old_cmd11) > 0.05) && fabs(old_cmd00)+fabs(old_cmd11) > 0.05 )
					{
						...
						int current_cmd_offset = cmd_offset;
						/*
						if(fabs(0.8 > sqrt(old_cmd0*old_cmd0 + old_cmd1*old_cmd1)) >= 0.5)
							current_cmd_offset = cmd_offset*2;
						else if(fabs(sqrt(old_cmd0*old_cmd0 + old_cmd1*old_cmd1)) >= 0.8)
							current_cmd_offset = cmd_offset*3;
						*/
						if (((fabs(sqrt(old_pitch*old_pitch + old_roll*old_roll))/1.44) >= 0.2)
							&& ((fabs(sqrt(old_pitch*old_pitch + old_roll*old_roll))/1.44) < 0.5))
